...
def make_linear_regressor(max_features, cv):
    ...
    logging.info("Starting make linear regressor")
    ...
    logging.info("Make linear regressor completed")
...
def make_knn_regressor(max_features, cv, max_neighbors):
    ...
    logging.info("Starting make knn regressor")
    ...
    logging.info("Make knn regressor completed")
...
def compute_metrics(y_true, y_pred):
    ...
    logging.info("Compute metrics completed")
    return mse, mae, r2
...
def load_model_from_disk():
    ...
    logging.info("Starting load model from disck")
    ...
    logging.info("Load model from disck completed")
    return model


def save_model_to_disk(model):
    """Save the model to the disk"""
    logging.info("Starting save model to disk")
    path = os.path.join(config["models_dir"], config["house_prices_model"])
    with open(path, "wb") as file:
        pickle.dump(model, file)
    logging.info("Save model to disk completed")


def compare_saved_model_with_new_model(saved_model, new_model):
    """Compare the current model with the new model"""

    logging.info("Starting compare saved model with new model")
    ...
    if new_model.best_score_ > saved_model.best_score_:
        ...
        logging.info("Compare saved model with new model completed")

        return new_model

    logging.info("Saved model completed")
...
def load_train_and_test_datasets():
    ...
    logging.info("Starting load train and test datasets")
    ...
    logging.info("Load train and test datasets completed")
